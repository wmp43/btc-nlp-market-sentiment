...
def clf_dev_master(db_name, table_name):
...
def perform_reg_grid_search(df, classifier_model_path):
    ...
    print("Best Directional Accuracy on validation set: ", grid_search.cv_results_['mean_test_Directional Accuracy'][grid_search.best_index_])

    # Save the best model as a pickle file
    joblib.dump(grid_search.best_estimator_, 'xgb_reg_1.0.pkl')

    return grid_search.best_params_, -grid_search.best_score_, -grid_search.cv_results_['mean_test_MAPE'][grid_search.best_index_]
...
def reg_dev_master(clf_path, db_name, table_name):
    """
    ...
    X_val = df_with_clf_preds.drop(columns=['mean_sentiment_score_LABEL_0', 'max_sentiment_score_LABEL_0', 
                ...
                'mean_sentiment_score_LABEL_1', 'max_sentiment_score_LABEL_1', 
                'min_sentiment_score_LABEL_1', 'std_sentiment_score_LABEL_1',
                'future_returns','time_published', 'index', 'titles', 'content']).iloc[split_idx:]
    y_val = df_with_clf_preds['future_returns'].iloc[split_idx:]

    best_model = joblib.load('xgb_reg_1.0.pkl')

    rmse, mape, mae, directional_accuracy = evaluate_model(best_model, X_val, y_val)
